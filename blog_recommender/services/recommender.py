...
from ..config.weights import RECOMMENDATION_WEIGHTS, SIMILARITY_THRESHOLD, SIMILAR_USERS_COUNT
...
class RecommenderService:
    ...
    def calculate_content_similarity(self, blog_id: int, other_blog_id: int) -> float:
        """Calculate content similarity between two blogs"""
        ...
        # Combine title, content, and tags for similarity calculation
        text1 = f"{blog1.title} {blog1.content} {' '.join(blog1.tags)}"
        text2 = f"{blog2.title} {blog2.content} {' '.join(blog2.tags)}"
    ...
    def find_similar_users(self, user_id: int) -> List[int]:
        ...
        return [user_id for user_id, _ in user_similarities[:SIMILAR_USERS_COUNT]]
    ...
    def get_recommendations(self, user_id: int, blog_id: int) -> List[int]:
        ...
        for blog in all_blogs:
            ...
            final_score = (
                ...
                RECOMMENDATION_WEIGHTS['content_similarity'] * content_similarity_score +
                RECOMMENDATION_WEIGHTS['user_similarity'] * user_similarity_score
            )
